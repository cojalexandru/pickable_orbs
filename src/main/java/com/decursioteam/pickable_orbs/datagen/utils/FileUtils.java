...
public class FileUtils {
    ...
    public static void streamFilesAndParse(Path directoryPath, BiConsumer<Reader, String> instructions,
            String errorMessage) {
        try (Stream<Path> zipStream = Files.walk(directoryPath);
            ...
            zipStream.filter(f -> f.getFileName().toString().endsWith(ZIP))
                    .forEach(path -> addZippedFile(path, instructions));
            jsonStream.filter(f -> f.getFileName().toString().endsWith(JSON))
                    .forEach(path -> addFile(path, instructions));
        } catch (IOException e) {
    ...
    public static void setupDefaultFiles(String dataPath, Path targetPath) {
        String[] parts = dataPath.startsWith("/") ? dataPath.substring(1).split("/") : dataPath.split("/");
        Path path = ModList.get().getModFileById(PickableOrbs.MOD_ID).getFile().findResource(parts);

        if (Files.exists(path)) {
            copyFiles(path, targetPath);
        } else {
            PickableOrbs.LOGGER.error("Could not find resource path: {}", dataPath);
        }
    }
    ...
    private static void parseType(ZipFile zf, ZipEntry zipEntry, BiConsumer<Reader, String> consumer)
            throws IOException {
